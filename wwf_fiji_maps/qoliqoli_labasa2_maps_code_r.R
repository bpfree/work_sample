###################################
# Labasa2 (eastern) District Report Maps
###################################
...
# scale bar
scalebar_Labasa2 <- annotation_scale(width_hint = 0.2, # percent of the plot occupied (20%)
                                     pad_x = unit(0.05, "in"), # how much padded from the x=0 position
                                     pad_y = unit(0.05, "in")) # how much padded from the y=0 position

# north arrow
narrow_Labasa2 <- annotation_north_arrow(height = unit(0.25, "in"), 
                                         width = unit(0.2, "in"),
                                         pad_x = unit(0.82, "in"),
                                         pad_y = unit(0.05, "in"),
...
# scale bar for sedimentation
scalebar_Labasa2_sediment <- annotation_scale(width_hint = 0.2, # percent of the plot occupied (20%)
                                     pad_x = unit(0.05, "in"), # how much padded from the x=0 position
                                     pad_y = unit(3.60, "in")) # how much padded from the y=0 position

# north arrow for sedimentation
narrow_Labasa2_sediment <- annotation_north_arrow(height = unit(0.25, "in"), 
                                         width = unit(0.2, "in"),
                                         pad_x = unit(0.9, "in"),
                                         pad_y = unit(3.45, "in"),
                                         style = north_arrow_orienteering(
...
# scale bar for sedimentation
scalebar_Labasa2_geomorphic <- annotation_scale(width_hint = 0.2, # percent of the plot occupied (20%)
                                              pad_x = unit(0.05, "in"), # how much padded from the x=0 position
                                              pad_y = unit(3.60, "in")) # how much padded from the y=0 position

# north arrow for sedimentation
narrow_Labasa2_geomorphic <- annotation_north_arrow(height = unit(0.25, "in"), 
                                                  width = unit(0.2, "in"),
                                                  pad_x = unit(0.9, "in"),
                                                  pad_y = unit(3.45, "in"),
                                                  style = north_arrow_orienteering(
...
# map themes
map_theme_Labasa2 <- theme(axis.text=element_text(size=8),
                           ...
                           legend.position =  c(0.20,0.92), # alternative = bottom
...
map_theme_Labasa2_geomorphic <- theme(axis.text=element_text(size=8),
                                      axis.title=element_text(size=10),
                                      plot.title=element_text(size=12),
                                      panel.grid.major = element_line(color = "transparent"), 
                                      panel.grid.minor = element_line(color = "transparent"),
                                      ...
                                      legend.position =  c(0.23,0.19), # alternative = bottom
                                      legend.text = element_text(size=5), # text size of the descriptor
                                      legend.background = element_rect(fill = "transparent"), # make the box transparent --> "transparent"
                                      legend.box.background = element_rect(fill = "white", color = "#4C84A2"), # white background with blue border
                                      legend.box.margin = margin(1,1,1,1), # add some space between the box and the text
                                      legend.spacing.y = unit(0.025, "in"),
                                      legend.key.size = unit(0.08, "in"), # size of the color box
                                      legend.key = element_rect(fill = "transparent"), # make the background of the key clear
                                      legend.margin = margin(0, 0, 0, 0, "in"), # reduce spacing between legend elements
                                      axis.line = element_line(colour = "black"))

map_theme_Labasa2_sediment <- theme(axis.text=element_text(size=8),
                                    axis.title=element_text(size=10),
                                    plot.title=element_text(size=12),
                                    panel.grid.major = element_line(color = "transparent"), 
                                    panel.grid.minor = element_line(color = "transparent"),
                                    ...
                                    legend.position =  c(0.90,0.85), # alternative = bottom
...
map_theme_Labasa2_bathymetry <- theme(axis.text=element_text(size=8),
                                      axis.title=element_text(size=10),
                                      plot.title=element_text(size=12),
                                      panel.grid.major = element_line(color = "transparent"), 
                                      panel.grid.minor = element_line(color = "transparent"),
                                      ...
                                      axis.text.y = element_text(angle = 90, hjust = 0.5),
                                      legend.title = element_text(size = 6), # size of the title
                                      legend.position =  c(0.20,0.20), # alternative = bottom
                                      legend.text = element_text(size=6), # text size of the descriptor
                                      legend.background = element_rect(fill = "transparent"), # make the box transparent --> "transparent"
                                      legend.box.background = element_rect(fill = "white", color = "#4C84A2"), # white background with blue border
                                      legend.box.margin = margin(1,1,1,1), # add some space between the box and the text
...
map_theme_Labasa2_survey <- theme(axis.text=element_text(size=8),
                                  axis.title=element_text(size=10),
                                  plot.title=element_text(size=12),
                                  panel.grid.major = element_line(color = "transparent"), 
                                  panel.grid.minor = element_line(color = "transparent"),
                                  ...
                                  legend.position =  c(0.20,0.25), # alternative = bottom
                                  legend.text = element_text(size = 6), # text size of the descriptor
                                  legend.background = element_rect(fill = "transparent"), # make the box transparent --> "transparent"
                                  legend.box.background = element_rect(fill = "white", color = "#4C84A2"), # white background with blue border
                                  legend.box.margin = margin(1,1,1,1), # add some space between the box and the text
...
for (i in 2){
  ...
  # get the limits
  xlim_qoli <- c(xmin = st_bbox(qoliqoli_do)$xmin-250, xmax = st_bbox(qoliqoli_do)$xmax+250)
  ylim_qoli <- c(ymin = st_bbox(qoliqoli_do)$ymin, ymax = st_bbox(qoliqoli_do)$ymax)
  
  # extract qoliqoli name
  qoli_name <- qoliqoli_do$District
  
  # x-axis limits
  if(i==2){xbreaks <- seq(179.38,179.44,0.02)} # Labasa2
  ...
  qoliqoli_geomorphic <- ggplot() + 
    ...
    labs(x="",y="",title="") + 
    # map elements
    scalebar_Labasa2_geomorphic +
    narrow_Labasa2_geomorphic +
    # theme
    theme_bw() + 
    map_theme_Labasa2_geomorphic
  ...
  qoliqoli_sediment <- ggplot() + 
    ...
    scale_x_longitude(breaks = xbreaks) +
    # map elements
    scalebar_Labasa2_sediment +
    narrow_Labasa2_sediment +
    # theme
    theme_bw() + 
    map_theme_Labasa2_sediment
